...
//selectez din baza de date doar inregistrarile din ultima zi.
  $result = $mysqli->query('SELECT * FROM airsense WHERE airsense.data>DATE_SUB(CURDATE(), INTERVAL 1 MONTH)');
...
$mysqli->close();
  ...
  $table['cols'] = array(
    ...
    array('label' => 'calitateaer', 'type' => 'number'),
...
$t[] = array('v' => (string) $r['data']); 
      $t[] = array('v' => (string) $r['calitateaer']); 
      $rows[] = array('c' => $t);
    }
...
//creeare tabel cu datele preluate (json)
      ...
      var options = {
           title: 'Evolutia calitatii aerului in ultima luna:',
          is3D: 'true',
          width: 1000,
          height: 250,
		  colors: ['#008000'],
